                ...
                <table width="600" border="0" cellspacing="0" cellpadding="0"
                    ...
                    <tr>
                        <td style="background-color: #2563eb; padding: 20px; text-align: center;">
                            <h1 style="color: #ffffff; font-size: 24px; margin: 0;">¡Hola, {{ $user->full_name }}!</h1>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 30px; color: #4b5563; line-height: 1.6;">
                            <p style="font-size: 16px; margin: 0 0 20px;">
                                ...
                                <strong>{{ config('app.name') }}</strong>. Para continuar, haz clic en el siguiente botón:
                            </p>
                            <p style="text-align: center; margin: 30px 0;">
                                <a href="{{ config('app.frontend_url') . '/olvide-contrasena?token=' . $token . '&email=' . urlencode($user->email) }}"
                            ...
                            <p style="font-size: 14px; color: #6b7280; margin: 20px 0;">
                                ...
                                <a href="{{ config('app.frontend_url') . '/olvide-contrasena?token=' . $token . '&email=' . urlencode($user->email) }}"
                                    ...
                                    {{ config('app.frontend_url') . '/olvide-contrasena?token=' . $token . '&email=' . urlencode($user->email) }}
                                </a>
                            </p>
                            <p style="font-size: 14px; color: #6b7280; margin: 20px 0;">
                                Este enlace expirará en 15 minutos. Si no solicitaste restablecer tu contraseña, ignora este correo o ponte en contacto con soporte.
                            </p>
                        </td>
                    </tr>
                    <tr>
