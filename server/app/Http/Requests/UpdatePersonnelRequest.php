...
class UpdatePersonnelRequest extends FormRequest
{
    ...
    {
        $personnelId = $this->route('personnel') ? $this->route('personnel')->id : null;

        return [
            'status' => ['nullable', Rule::in(['active', 'inactive'])],
            'type_personnel_id' => 'nullable|integer|exists:type_personnels,id',
            'nac' => ['required', Rule::in(['V', 'E'])],
            'ci' => [
                'required',
                'string',
                'max:20',
                Rule::unique('personnels', 'ci')->ignore($personnelId)
            ],
